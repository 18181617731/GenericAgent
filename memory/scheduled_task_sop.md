...
- 冷却期内的正常跳过通常不记详细日志；`scheduler.log` 长时间不变化，不等于调度进程没跑

## 注意
- once类型：执行一次后冷却100年（实际效果为永久跳过）
- 任务文件只管"干什么"，报告路径由scheduler自动生成注入prompt
- sche_tasks目录在../，即code root下
- `daily` / `weekday` 当前按约20小时冷却；是否已执行看 `sche_tasks/done/*_{任务ID}.md` 的最近报告时间
- 仅修改 `schedule` 不修改任务ID时，旧 done 报告仍会参与冷却判定；若任务从晚间改到早间，可能被前一晚的执行记录挡住
- 排查“到点没触发”时，先核对三件事：调度进程是否在跑、对应任务ID最近 done 时间、当前是否仍在冷却/是否已超过 `max_delay_hours` 窗口